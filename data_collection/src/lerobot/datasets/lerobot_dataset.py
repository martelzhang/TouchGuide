...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        """
        Query the HF dataset for indices across keys, skipping video keys.
        Tries column-first [key][index] for speed, falls back to row-first.
        
        Args:
            query_indices (dict[str, list[int]]): Dictionary mapping keys to index lists to retrieve.

        Returns:
            dict: Dictionary with stacked tensors of queried data (video keys excluded).
        """
        result: dict = {}
