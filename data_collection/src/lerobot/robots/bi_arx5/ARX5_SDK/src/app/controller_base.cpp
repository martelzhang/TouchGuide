...
void Arx5ControllerBase::reset_to_home()
{
    ...
    double home_tolerance = 0.065; // rad tolerance
    bool near_home = max_pos_error < home_tolerance;
    
    if (near_home) {
        // logger_->info("Already near home position (error: {:.3f}), using single-stage reset", max_pos_error);
        // near home position, set all joints to 0, skip intermediate position
        target_state.timestamp = get_timestamp() + wait_time;
        target_state.pos[2] = 0.0; // set to 0, no intermediate position
    } else {
        // logger_->info("Far from home position (error: {:.3f}), using two-stage reset", max_pos_error);
        // far from home position, use two-stage reset
        target_state.timestamp = get_timestamp() + wait_time;
    ...
    // second stage, only execute when far from home
    if (!near_home) {
...
void Arx5ControllerBase::update_joint_state_()
{
    ...
    joint_state_.gripper_torque =
        motor_msg[robot_config_.gripper_motor_id].current_actual_float * torque_constant_DM_J4310; // TODO: check if this is correct
...
void Arx5ControllerBase::send_recv_()
{
    ...
    if (robot_config_.gripper_motor_type == MotorType::DM_J4310)
    {
        ...
        double gripper_motor_pos = output_joint_cmd_.gripper_pos / robot_config_.gripper_width * robot_config_.gripper_open_readout;
        double gripper_motor_vel = output_joint_cmd_.gripper_vel / robot_config_.gripper_width * robot_config_.gripper_open_readout;
